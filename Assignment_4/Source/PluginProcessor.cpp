...
VibratoPluginAudioProcessor::~VibratoPluginAudioProcessor()
{
    CVibrato::destroyInstance(pVibratoInstance);
}
...
void VibratoPluginAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    ppfAudioData = new float*[totalNumInputChannels];

    pVibratoInstance->initInstance(fDefaultMaxModWidthInS, sampleRate, totalNumInputChannels);
    pVibratoInstance->setParam(CVibrato::kParamModWidthInS, fDefaultModWidthInS);
    pVibratoInstance->setParam(CVibrato::kParamModFreqInHz, fDefaultModFreqInHz);
...
void VibratoPluginAudioProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& midiMessages)
{
    ...
    {
        ppfAudioData[channel] = buffer.getWritePointer (channel);
    }
    pVibratoInstance->process(ppfAudioData, ppfAudioData, buffer.getNumSamples());
}
